...
namespace Faro
{
    ...
    void GraphicsBufferD3D12::Destroy()
    {
    ...
    void GraphicsBufferD3D12::TransitionResource(ID3D12GraphicsCommandList* commandList, GraphicsResourceState state)
    {
    ...
    ID3D12Resource* GraphicsBufferD3D12::GetResource()
    {
        return gpuResource;
    }

    D3D12_CPU_DESCRIPTOR_HANDLE GraphicsBufferD3D12::GetDescriptor()
    {
        if (descriptorHeap != nullptr)
        {
    ...
    void GraphicsBufferD3D12::Upload(uint8* data)
    {
        MemoryManager::Copy<uint8>(data, cpuAddress, GetDesc().dataSize);
    }

    void GraphicsBufferD3D12::Init()
    {
        IGraphicsAdapterChild::Init();

        if (GetBufferType() == BT_Upload)
        {
    ...
    D3D12_RESOURCE_DESC GraphicsBufferD3D12::GetNativeDesc()
    {
